...
public class ProductHibernateDaoTest extends AbstractHibernateDaoTest {
    ...
    @Test
    @ExpectedException(EntityNotFoundException.class)
    public void getByInvalidProductCode() {
	String productCodeToSearch = "INVALIDPRODUCT";
	productDao.getByProductCode(productCodeToSearch);
    }
	...
	String productCodeToSearch = "PRODUCT3";
	Product product = productDao.getByProductCode(productCodeToSearch);
	assertNotNull(product);
	assertThat(product.getProductCode(), equalTo(productCodeToSearch));
    }
